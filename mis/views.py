...
def login_view(request):
    ...
    if request.method == 'POST':
        ...
        if UserSiteMapping.objects.filter(site__domain = data).exists():
...
# def get_fossil_dl_session_name(request, awc_id):
#     if request.method == 'GET' and request.is_ajax():
#         result_set = []
#         fossildlsessions = FossilDLSessionConfig.objects.filter(status=1, session_category__id=awc_id)
#         for fossildlsession in fossildlsessions:
#             result_set.append(
#                 {'id': fossildlsession.id, 'name': fossildlsession.get_session_type_display(),})
#         return HttpResponse(json.dumps(result_set))
...
def add_sessions_monitoring_fossil_po_report(request, task_id):
    ...
    awc_obj = AWC.objects.filter(status=1, id__in=awc_id)
    village_obj = Village.objects.filter(status=1, id__in=village_id)
    school_obj = School.objects.filter(status=1, id__in=school_id)
    if request.method == 'POST':
        data = request.POST
